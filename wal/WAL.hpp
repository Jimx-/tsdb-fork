...
class Segment: boost::noncopyable{
    ...
    public:
        ...
        Segment(const std::string & filename, const std::string & dir, int index); // Read.
...
class SegmentRange{
    public:
        ...
        SegmentRange(const std::string & dir, int first, int last): dir(dir), first(first), last(last){}
};
...
class CorruptionError{
    public:
        ...
        CorruptionError(const std::string dir, int segment, uint64_t offset, const error::Error & err_)
...
class WAL: boost::noncopyable{
    ...
    public:
        WAL(const std::string & dir, const std::shared_ptr<base::ThreadPool> & pool_, int segment_size=SEGMENT_SIZE);
