...
// last_checkpoint returns the directory name and index of the most recent checkpoint.
// If dir does not contain any checkpoints, ErrNotFound is returned.
std::pair<std::pair<std::string, int>, error::Error> last_checkpoint(const std::string & dir){
    std::string r;
    int idx = -1;
    ...
    for(auto const& entry : boost::make_iterator_range(boost::filesystem::directory_iterator(p), {})){
        if(boost::filesystem::is_directory(entry.path()) && entry.path().filename().string().length() >= CHECKPOINT_PREFIX.length() && entry.path().filename().string().substr(0, CHECKPOINT_PREFIX.length()) == CHECKPOINT_PREFIX){
            std::string temp = entry.path().filename().string().substr(CHECKPOINT_PREFIX.length());
...
// delete_checkpoints deletes all checkpoints in a directory below a given index.
...
std::pair<CheckpointStats, error::Error> checkpoint(WAL * wal, int from, int to, const boost::function<bool (uint64_t)> & keep, int64_t mint){
    CheckpointStats stats;
    std::deque<SegmentRange> seg_ranges;
    std::pair<std::pair<std::string, int>, error::Error> lp = last_checkpoint(wal->dir());
    ...
    if(!lp.second){
        if(from > last)
            return {CheckpointStats(), error::Error("unexpected gap to last checkpoint. expected:" + std::to_string(last) + ", requested:" + std::to_string(from))};
    ...
    if(reader.error())
        return {CheckpointStats(), error::wrap(reader.error(), "create segment reader")};

    std::string cpdir = tsdbutil::filepath_join(wal->dir(), (boost::format(CHECKPOINT_PREFIX + "%06d") % to).str());
    std::string cpdirtmp = cpdir + ".tmp";

    {
        WAL cp_wal(cpdirtmp, wal->pool());
        if(cp_wal.error())
            return {CheckpointStats(), error::wrap(cp_wal.error(), "open checkpoint")};
        ...
        while(reader.next()){
            ...
            tsdbutil::RECORD_ENTRY_TYPE type = tsdbutil::RecordDecoder::type(rec.first, rec.second);
            // std::cerr << "type: " << (int)(rec.first[0]) << ", " << (int)(type) << std::endl;
            if(type == tsdbutil::RECORD_SERIES){
                error::Error err = tsdbutil::RecordDecoder::series(rec.first, rec.second, series);
                if(err)
                    return {CheckpointStats(), error::wrap(err, "decode series")};
            ...
            //     error::Error err = tsdbutil::RecordDecoder::group_series(rec.first, rec.second, group_series);
            //     if(err)
            //         return {CheckpointStats(), error::wrap(err, "decode group series")};
            ...
            //         recs.push_back(std::vector<uint8_t>());
            //         tsdbutil::RecordEncoder::group_series(group_series, recs.back());
            //         count += recs.back().size();
            //     }
            //     stats.dropped_series += rm_count;
            // }
            else if(type == tsdbutil::RECORD_SAMPLES){
                error::Error err = tsdbutil::RecordDecoder::samples(rec.first, rec.second, samples);
                if(err)
                    return {CheckpointStats(), error::wrap(err, "decode samples")};
            ...
            //     error::Error err = tsdbutil::RecordDecoder::group_samples(rec.first, rec.second, group_samples);
            //     if(err)
            //         return {CheckpointStats(), error::wrap(err, "decode group samples")};
            ...
            //         recs.push_back(std::vector<uint8_t>());
            //         tsdbutil::RecordEncoder::group_samples(group_samples, recs.back());
            //         count += recs.back().size();
            //     }
            //     stats.dropped_samples += rm_count;
            // }
            else if(type == tsdbutil::RECORD_TOMBSTONES){
                error::Error err = tsdbutil::RecordDecoder::tombstones(rec.first, rec.second, stones);
                if(err)
            ...
            //     error::Error err = tsdbutil::RecordDecoder::group_tombstones(rec.first, rec.second, stones);
            //     if(err)
            //         return {CheckpointStats(), error::wrap(err, "decode group tombstones")};
            //     stats.total_tombstones += stones.size();
            //     int rm_count = 0;
            //     auto it = stones.begin();
            //     while(it != stones.end()){
            ...
            //     if(!stones.empty()){
            //         recs.push_back(std::vector<uint8_t>());
            //         tsdbutil::RecordEncoder::group_tombstones(stones, recs.back());
            //         count += recs.back().size();
            //     }
            //     stats.dropped_tombstones += rm_count;
            // }
            ...
            if(count > 1 * 1024 * 1024){
                ...
                if(err)
                    return {CheckpointStats(), error::wrap(err, "flush records")};
                count = 0;
                recs.clear();
            }
        }
