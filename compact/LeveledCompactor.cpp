...
std::pair<std::deque<std::string>, error::Error>
LeveledCompactor::plan_helper(const std::shared_ptr<block::DirMetas>& dms)
{
    ...
    // We do not include a recently created block with max(minTime), so the
    // block which was just created from WAL. This gives users a window of a
    // full block size to piece-wise backup new data without having to care
    // about data overlap.
    dms->pop_back();
...
std::pair<std::deque<std::string>, error::Error>
LeveledCompactor::plan(const std::string& dir)
{
    ...
    for (const std::string& s : dirs) {
        ...
        dms->push_back(block::DirMeta(s, std::shared_ptr<block::BlockMeta>(
                                             new block::BlockMeta(p.first))));
    }
...
LeveledCompactor::overlapping_dirs(const std::shared_ptr<block::DirMetas>& dms)
{
...
std::shared_ptr<block::DirMetas>
LeveledCompactor::select_dirs(const std::shared_ptr<block::DirMetas>& dms)
{
    ...
    for (int i = 1; i < ranges.size(); i++) {
        ...
        for (auto const& part : parts) {
            // Do not select the range if it has a block whose compaction
            // failed.
            // TODO(Alec)
            bool jump = false;
            ...
            // Pick the range of blocks if it spans the full range (potentially
            // with gaps) or is before the most recent block. This ensures we
            // don't compact blocks prematurely when another one of the same
            // size still fits in the range.
            //
            // Prerequisite: [1]. no overlapping blocks [2]. sorted by min_time
            // 1. number of blocks must > 1.
            // 2. the most recent block(the popped one in plan()) may reside in
            // range of last part.
...
std::deque<std::shared_ptr<block::DirMetas>>
LeveledCompactor::split_by_range(const std::shared_ptr<block::DirMetas>& dms,
                                 int64_t range)
{
    std::deque<std::shared_ptr<block::DirMetas>> parts;
...
LeveledCompactor::compact(const std::string& dest,
                          ...
                          const std::shared_ptr<block::Blocks>& open)
{
    ...
    for (auto const& d : dirs) {
        ...
        if (!meta_pair.second)
            return {ulid::ULID(),
                    error::Error("Error reading block meta: " + d)};
        ...
        if (!b) {
            ...
            if (b->error())
                return {ulid::ULID(),
                        error::Error("Error opening block: " + d)};
        }
        blocks->push_back(b);
        ...
        if (compacted_meta.stats.num_samples == 0) {
            for (int i = 0; i < blocks->size(); ++i) {
                if (!blocks->at(i)->set_deletable())
                    LOG_ERROR << "msg=\"Failed to write 'deletable' to meta "
    ...
    const ulid::ULID& ulid, const std::shared_ptr<block::BlockMetas>& bms)
{
    ...
    for (int i = 0; i < bms->size(); i++) {
        ...
        bm.compaction.parents.emplace_back(
            bms->at(i).ulid_, bms->at(i).min_time, bms->at(i).max_time);
    }
    ++bm.compaction.level;
    ...
    const std::shared_ptr<block::ChunkWriterInterface>& chunkw)
{
    ...
    for (int i = 0; i < blocks->size(); i++) {
        ...
        if (!overlapping) {
            if (i > 0 && blocks->at(i)->MinTime() < max_time) {
                overlapping = true;
                LOG_WARN << "found overlapping blocks during "
                            "populate_blocks(), ulid: "
        ...
        std::pair<std::shared_ptr<block::ChunkReaderInterface>, bool>
            chunks_pair = blocks->at(i)->chunks();
        ...
        std::pair<std::unique_ptr<index::PostingsInterface>, bool>
            all_postings_pair = index_pair.first->get_all_postings();
        ...
        sets->push_back(std::shared_ptr<querier::ChunkSeriesSetInterface>(
            new CompactionChunkSeriesSet(index_pair.first, chunks_pair.first,
                                         tombstones_pair.first,
    ...
    while (mcss.next()) {
        ...
        if (overlapping) {
            // If blocks are overlapping, it is possible to have unsorted
            // chunks.
        ...
        for (int i = 0; i < csm->chunks.size(); ++i) {
            ...
            if (csm->chunks[i]->min_time < bm->min_time ||
                ...
                return error::Error(
                    ...
                    " outside of compacted minTime: " +
                    std::to_string(bm->min_time) +
                    " maxTime: " + std::to_string(bm->max_time));
            // Re-encode the chunk to not have deleted values.
            if (!csm->intervals.empty()) {
                if (!csm->chunks[i]->overlap_closed(
                        csm->intervals.front().min_time,
                ...
                chunk::DeleteIterator it(
                    std::move(csm->chunks[i]->chunk->iterator()),
                    csm->intervals.cbegin(), csm->intervals.cend());
                while (it.next()) {
        ...
        if (overlapping) {
            auto merged_chunks = chunk::merge_overlapping_chunks(csm->chunks);
            if (merged_chunks.second)
                return error::wrap(merged_chunks.second,
        ...
        if ((err = indexw->add_series(csm->tsid, csm->chunks)) !=
            index::SUCCEED)
            return error::wrap(error::Error(index::error_string(err)),
                               "add_series");
...
error::Error
LeveledCompactor::write_helper(const std::string& dest, block::BlockMeta* bm,
                               const std::shared_ptr<block::Blocks>& blocks)
{
...

    {
        ...
        {
            // Populate chunk and index files into temporary directory with data
            // of all blocks.
            std::shared_ptr<block::ChunkWriterInterface> chunkw =
                std::shared_ptr<chunk::ChunkWriter>(
                    new chunk::ChunkWriter(tmp.string() + "/chunks"));

            std::shared_ptr<block::IndexWriterInterface> indexw =
                std::shared_ptr<index::IndexWriter>(
                    new index::IndexWriter(tmp.string() + "/index"));
...
std::pair<ulid::ULID, error::Error>
LeveledCompactor::write(const std::string& dest,
                        ...
                        const std::shared_ptr<block::BlockMeta>& parent)
{
    ...
    if (bm.stats.num_samples == 0) return {ulid::ULID(), error::Error()};
