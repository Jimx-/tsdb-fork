...
LeveledCompactor::LeveledCompactor(const std::deque<int64_t> & ranges, const std::shared_ptr<base::Channel<char>> & cancel): ranges(ranges), cancel(cancel){
    if(ranges.empty())
        err_.set("at least one range must be provided");
}
LeveledCompactor::LeveledCompactor(const std::vector<int64_t> & ranges, const std::shared_ptr<base::Channel<char>> & cancel): ranges(ranges.begin(), ranges.end()), cancel(cancel){
    if(this->ranges.empty())
        err_.set("at least one range must be provided");
}
LeveledCompactor::LeveledCompactor(const std::initializer_list<int64_t> & ranges, const std::shared_ptr<base::Channel<char>> & cancel): ranges(ranges.begin(), ranges.end()), cancel(cancel){
    if(this->ranges.empty())
        err_.set("at least one range must be provided");
}

std::pair<std::deque<std::string>, error::Error> LeveledCompactor::plan_helper(const std::shared_ptr<block::DirMetas> & dms){
    ...
    for(int i = dms->size() - 1; i >= 0; i --){
        if(dms->at(i).meta->max_time - dms->at(i).meta->min_time < ranges[ranges.size() / 2])
            break;
        if((double)(dms->at(i).meta->stats.num_tombstones) / (double)(dms->at(i).meta->stats.num_series) > 0.05){
...
std::pair<std::deque<std::string>, error::Error> LeveledCompactor::plan(const std::string & dir){
    std::deque<std::string> dirs = db::block_dirs(dir);
    if(dirs.empty())
        return {dirs, error::Error()};
    ...
    for(const std::string& s: dirs){
        ...
        if(!p.second)
            return {std::deque<std::string>(), error::Error("Error reading block meta")};
        dms->push_back(block::DirMeta(s, std::shared_ptr<block::BlockMeta>(new block::BlockMeta(p.first))));
    }
...
std::deque<std::string> LeveledCompactor::overlapping_dirs(const std::shared_ptr<block::DirMetas> & dms){
    if(dms->size() < 2)
    ...
    for(int i = 1; i < dms->size(); i ++){
        ...
        if(dms->at(i).meta->max_time > max)
            max = dms->at(i).meta->max_time;
    }
    return overlappings;
}
...
std::shared_ptr<block::DirMetas> LeveledCompactor::select_dirs(const std::shared_ptr<block::DirMetas> & dms){
    ...
    for(int i = 1; i < ranges.size(); i ++){
        std::deque<std::shared_ptr<block::DirMetas> > parts = split_by_range(dms, ranges[i]);
        if(parts.empty())
            continue;
        ...
        for(auto const& part: parts){
            ...
            // 2. the most recent block(the popped one in plan()) may reside in range of last part.
            if((max_time - min_time == ranges[i] || max_time <= high_time) && part->size() > 1)
                return part;
...
std::deque<std::shared_ptr<block::DirMetas> > LeveledCompactor::split_by_range(const std::shared_ptr<block::DirMetas> & dms, int64_t range){
...
std::pair<ulid::ULID, error::Error> LeveledCompactor::compact(const std::string & dest, 
        const std::deque<std::string> & dirs, 
    ...
    if(!err){
        ...
        for(const std::string & uid: ulids)
            ulids_string += uid + ", ";
        ulids_string += "]";
        ...
        if(compacted_meta.stats.num_samples == 0){
            ...
            LOG_INFO    << "msg=\"compact blocks\" "
                        ...
                        << " max_time=" << compacted_meta.max_time
                        << " ulid=\"" << ulid::Marshal(compacted_meta.ulid_) << "\" "
                        << "sources=" << ulids_string
                        << " duration=" << base::timeDifference(base::TimeStamp::now(), start);
    ...
    if(err != "cancel"){
        for(int i = 0; i < blocks->size(); ++ i){
            if(blocks->at(i)->set_compaction_failed())
                err.wrap("setting compaction failed for block: " + blocks->at(i)->dir());
        }
    }
...
block::BlockMeta LeveledCompactor::compact_block_metas(const ulid::ULID & ulid, 
        const std::shared_ptr<block::BlockMetas> & bms){

    block::BlockMeta bm;
    bm.ulid_ = ulid;
    bm.min_time = bms->front().min_time;

    std::unordered_set<ulid::ULID> sources;
    int64_t max_time = std::numeric_limits<int64_t>::min();

    for(int i = 0; i < bms->size(); i ++){
        if(bms->at(i).max_time > max_time)
            max_time = bms->at(i).max_time;
        ...
        bm.compaction.parents.emplace_back(bms->at(i).ulid_, bms->at(i).min_time, bms->at(i).max_time);
    }
    ++ bm.compaction.level;

    bm.compaction.sources.insert(bm.compaction.sources.end(), sources.begin(), sources.end());

    std::sort(bm.compaction.sources.begin(), bm.compaction.sources.end(), [](const ulid::ULID & lhs, const ulid::ULID & rhs){
...
// populate_blocks fills the index and chunk writers with new data gathered as the union
...
// ------- 1.2.4. std::unique_ptr<index::PostingsInterface> which contains label::ALL_POSTINGS_KEYS
...
error::Error LeveledCompactor::populate_blocks(const std::shared_ptr<block::Blocks> & blocks,
        block::BlockMeta * bm,
    ...
    std::shared_ptr<querier::ChunkSeriesSets> sets(new querier::ChunkSeriesSets());
    ...
    for(int i = 0; i < blocks->size(); i ++){
        ...
        if(!overlapping){
            if(i > 0 && blocks->at(i)->MinTime() < max_time){
                overlapping = true;
                LOG_WARN << "found overlapping blocks during populate_blocks(), ulid: " << ulid::Marshal(bm->ulid_);
            }
            if(blocks->at(i)->MaxTime() > max_time)
                max_time = blocks->at(i)->MaxTime();
        }

        std::pair<std::shared_ptr<block::IndexReaderInterface>, bool> index_pair = blocks->at(i)->index();
        if(!index_pair.second)
            return error::Error("Error open index reader for block " + ulid::Marshal(blocks->at(i)->meta().ulid_));

        std::pair<std::shared_ptr<block::ChunkReaderInterface>, bool> chunks_pair = blocks->at(i)->chunks();
        if(!chunks_pair.second)
            return error::Error("Error open chunks reader for block " + ulid::Marshal(blocks->at(i)->meta().ulid_));

        std::pair<std::shared_ptr<tombstone::TombstoneReaderInterface>, bool> tombstones_pair = blocks->at(i)->tombstones();
        if(!tombstones_pair.second)
            return error::Error("Error open tombstone reader for block " + ulid::Marshal(blocks->at(i)->meta().ulid_));

        const std::deque<std::string> & temp_symbols = index_pair.first->symbols_deque();
        
        symbols.insert(temp_symbols.begin(), temp_symbols.end());

        std::pair<std::unique_ptr<index::PostingsInterface>, bool> all_postings_pair = index_pair.first->postings(label::ALL_POSTINGS_KEYS.label, label::ALL_POSTINGS_KEYS.value);
        if(!all_postings_pair.second)
            return error::Error("Error get postings of ALL_POSTINGS_KEYS " + ulid::Marshal(blocks->at(i)->meta().ulid_));

        // Append block to ChunkSeriesSets.
        sets->push_back(std::shared_ptr<querier::ChunkSeriesSetInterface>(
            new CompactionChunkSeriesSet(
                index_pair.first, chunks_pair.first, tombstones_pair.first, std::move(index_pair.first->sorted_postings(std::move(all_postings_pair.first)))
            )
        ));
    }

    // Create MergedChunkSeriesSet.
    MergedChunkSeriesSet mcss(sets);

    index::MemPostings postings;
    std::unordered_map<std::string, std::unordered_set<std::string>> label_values;
    uint64_t id = 0;

    // STEP 1 in index writer.
    if((err = indexw->add_symbols(symbols)) != index::SUCCEED)
        return error::wrap(error::Error(index::error_string(err)), "Error add symbols to index writer");
    ...
    while(mcss.next()){
        ...
        if(overlapping){
            ...
            std::sort(csm->chunks.begin(), csm->chunks.end(), [](const std::shared_ptr<chunk::ChunkMeta> & lhs, const std::shared_ptr<chunk::ChunkMeta> & rhs){
                return lhs->min_time < rhs->min_time;
            });
        }
        // Skip the series with all deleted chunks.
        // LOG_DEBUG << csm->chunks.size();
        if(csm->chunks.empty())
            continue;

        for(int i = 0; i < csm->chunks.size(); ++ i){
            // LOG_DEBUG << csm->chunks[i]->min_time << " " << csm->chunks[i]->max_time;
            if(csm->chunks[i]->min_time < bm->min_time || csm->chunks[i]->max_time > bm->max_time)
                return error::Error("found chunk with minTime: "
                    + std::to_string(csm->chunks[i]->min_time)
                    + " maxTime: "
                    + std::to_string(csm->chunks[i]->max_time)
                    + " outside of compacted minTime: " + std::to_string(bm->min_time)
                    + " maxTime: " + std::to_string(bm->max_time));
            // Re-encode the chunk to not have deleted values.
            if(!csm->intervals.empty()){
                if(!csm->chunks[i]->overlap_closed(csm->intervals.front().min_time, csm->intervals.back().max_time))
                    continue;
                ...
                chunk::DeleteIterator it(std::move(csm->chunks[i]->chunk->iterator()), csm->intervals.cbegin(), csm->intervals.cend());
                while(it.next()){
        ...
        if(overlapping){
            std::pair<std::deque<std::shared_ptr<chunk::ChunkMeta> >, error::Error> merged_chunks = chunk::merge_overlapping_chunks(csm->chunks);
        ...
        if((err = indexw->add_series(id, csm->lset, csm->chunks)) != index::SUCCEED)
        ...
        for(auto const& l: csm->lset)
            label_values[l.label].insert(l.value);

        // Add postings to MemPostings.
        postings.add(id, csm->lset);
    ...
    // STEP 3 in index writer.
    for(auto const& lv: label_values){
        if((err = indexw->write_label_index({lv.first}, std::deque<std::string>(lv.second.begin(), lv.second.end()))) != index::SUCCEED)
            return error::wrap(index::error_string(err), "write label index");
    }

    label::Labels lset = postings.sorted_keys();
    for(auto const& l: lset){
        // STEP 4 in index writer.
        std::unique_ptr<index::PostingsInterface> p = postings.get(l.label, l.value);
        if((err = indexw->write_postings(l.label, l.value, p.get())) != index::SUCCEED)
            return error::wrap(index::error_string(err), "write postings");
    }
    return error::Error();
}
...
error::Error LeveledCompactor::write_helper(const std::string & dest,
        block::BlockMeta * bm,
        const std::shared_ptr<block::Blocks> & blocks){

    // Clean and create dir dest/ulid_.tmp
    boost::filesystem::path dir = boost::filesystem::path(dest) / boost::filesystem::path(ulid::Marshal(bm->ulid_));
...

    {
        ...
        {
            // Populate chunk and index files into temporary directory with data of all blocks.
            std::shared_ptr<block::ChunkWriterInterface> chunkw = std::shared_ptr<chunk::ChunkWriter>(new chunk::ChunkWriter(tmp.string() + "/chunks"));

            std::shared_ptr<block::IndexWriterInterface> indexw = std::shared_ptr<index::IndexWriter>(new index::IndexWriter(tmp.string() + "/index"));
...
std::pair<ulid::ULID, error::Error> LeveledCompactor::write(const std::string & dest, 
        const std::shared_ptr<block::BlockInterface> & b,
        int64_t min_time,
        int64_t max_time, 
    ...
    if(parent){
        bm.compaction.parents.emplace_back(parent->ulid_, parent->min_time, parent->max_time);
    }

    error::Error err = write_helper(dest, &bm, std::shared_ptr<block::Blocks>(new block::Blocks({b})));
    if(err)
        return {ulid, error::wrap(err, "write_helper")};
    ...
    LOG_INFO    << "msg=\"write blocks\" "
                << "mint=" << bm.min_time
                << " maxt=" << bm.max_time
                << " ulid=\"" << ulid::Marshal(bm.ulid_) << "\" duration=" << base::timeDifference(base::TimeStamp::now(), start);
