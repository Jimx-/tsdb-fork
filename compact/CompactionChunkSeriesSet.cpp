...
CompactionChunkSeriesSet::CompactionChunkSeriesSet(const std::shared_ptr<block::IndexReaderInterface> & ir,
        const std::shared_ptr<block::ChunkReaderInterface> & cr,
        const std::shared_ptr<tombstone::TombstoneReaderInterface> & tr,
        std::unique_ptr<index::PostingsInterface> && p): p(std::move(p)), ir(ir), cr(cr), tr(tr), csm(new querier::ChunkSeriesMeta()), err_(){}
...
bool CompactionChunkSeriesSet::next() const{
    ...
    if(!csm->intervals.empty()){
        std::deque<std::shared_ptr<chunk::ChunkMeta> >::iterator it = csm->chunks.begin();
        while(it != csm->chunks.end()){
            if(tombstone::is_subrange((*it)->min_time, (*it)->max_time, csm->intervals))
...
const std::shared_ptr<querier::ChunkSeriesMeta> & CompactionChunkSeriesSet::at() const{
    return csm;
}
...
error::Error CompactionChunkSeriesSet::error_detail() const{
    return err_;
}
