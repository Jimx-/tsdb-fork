...
bool write_tombstones(const std::string& dir, TombstoneReaderInterface* tr)
{
    ...
    // Iterate writing.
    if (tr != NULL)
        tr->iter([f, &enc_buf, &crc32](const common::TSID& tsid,
                                       const Intervals& itv) {
                      ...
                      const std::shared_ptr<TombstoneReaderInterface>& tr)
{
    ...
    // Iterate writing.
    tr->iter(
        [&enc_buf, &crc32, f](const common::TSID& tsid, const Intervals& itv) {
            write_ts_helper(f, enc_buf, crc32, tsid, itv);
        });
...
std::pair<std::shared_ptr<TombstoneReaderInterface>, uint64_t>
read_tombstones(const std::string& dir)
{
    std::string p = tsdbutil::filepath_join(dir, "tombstones");
    ...
    while (dec_buf.len() > 0) {
        auto tsid = dec_buf.get_tsid();
        min_time = dec_buf.get_signed_variant();
        max_time = dec_buf.get_signed_variant();
        if (dec_buf.error() != tsdbutil::NO_ERR)
            return std::make_pair(std::shared_ptr<TombstoneReaderInterface>(),
                                  0);
        stones->add_interval(tsid, Interval(min_time, max_time));
    }
    return std::pair<std::shared_ptr<TombstoneReaderInterface>, uint64_t>(
        stones, bs.len());
}
