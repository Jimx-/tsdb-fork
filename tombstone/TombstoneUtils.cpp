...
void write_ts_helper(FILE * f, tsdbutil::EncBuf & enc_buf, base::CRC32 & crc32, uint64_t ref, const Intervals & itvs){
...
bool write_tombstones(const std::string & dir, TombstoneReaderInterface * tr){
    ...
    // Iterate writing.
    if(tr != NULL)
        tr->iter(static_cast<boost::function<void (uint64_t, const Intervals &)>>(boost::bind(&write_ts_helper, f, boost::ref(enc_buf), boost::ref(crc32), _1, _2)));
...
bool write_tombstones(const std::string & dir, const std::shared_ptr<TombstoneReaderInterface> & tr){
    std::string p = tsdbutil::filepath_join(dir, "tombstones");
    std::string tmp_path(p + std::string(".tmp"));
    FILE * f;
    if(!(f = fopen(tmp_path.c_str(), "wb")))
...
std::pair<std::shared_ptr<TombstoneReaderInterface>, uint64_t> read_tombstones(const std::string & dir){
    std::string p = tsdbutil::filepath_join(dir, "tombstones");

    if(!boost::filesystem::exists(p))
        return std::pair<std::shared_ptr<TombstoneReaderInterface>, uint64_t>(std::shared_ptr<TombstoneReaderInterface>(new MemTombstones()), 0);
    ...
    std::shared_ptr<TombstoneReaderInterface> stones = std::shared_ptr<TombstoneReaderInterface>(new MemTombstones());
    ...
    return std::pair<std::shared_ptr<TombstoneReaderInterface>, uint64_t>(stones, bs.len());
}
