...
Interval::Interval(int64_t min_time, int64_t max_time): min_time(min_time), max_time(max_time){}
...
void itvls_add(Intervals & itvs, const Interval & itvl){
    for(std::list<Interval>::iterator i = itvs.begin(); i != itvs.end(); i ++){
        ...
        if((*i).in_bounds(itvl.min_time) || (*i).in_bounds(itvl.min_time - 1)){
            ...
            if((*i).max_time < itvl.max_time)
                (*i).max_time = itvl.max_time;
            while(cur != itvs.end()){
                if((*cur).min_time > itvl.max_time + 1)
                    break;
                ++cur;
            }
        ...
        else if((*i).in_bounds(itvl.max_time) || (*i).in_bounds(itvl.max_time + 1)){
