...
Label::Label(const std::string & label, const std::string & value): label(label), value(value){}
...
Labels lbs_del(const Labels & lset, const std::string & name){
    Labels r;
    for(int i = 0; i < lset.size(); ++ i){
        if(lset[i].label != name)
            r.emplace_back(lset[i].label, lset[i].value);
    }
    return r;
}
...
uint64_t lbs_hash_for_labels(const Labels & lbs, const std::deque<std::string> & names){
    std::string input;
...
uint64_t lbs_hash_without_labels(const Labels & lbs, const std::deque<std::string> & names){
    std::string input;
    for(auto const& l: lbs){
        if(l.label == "__name__")
            continue;
...
Labels lbs_from_map(const std::unordered_map<std::string, std::string> & m){
    Labels r;
    for(std::unordered_map<std::string, std::string>::const_iterator it = m.cbegin(); it != m.cend(); it ++){
