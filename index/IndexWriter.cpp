...
IndexWriter::IndexWriter(const std::string & filename): pos(0), stage(IDX_STAGE_NONE), buf1(1 << 22), buf2(1 << 22), uint32_cache(1 << 15){
    ...
    symbols = std::unordered_map<std::string, uint32_t>();
...
int IndexWriter::ensure_stage(IndexWriterStage s){
    ...
    if(s < stage){
        LOG_ERROR << "Invalid stage: " << stage_to_string(s) << ", current at " << stage_to_string(stage); 
        return -1;
    }
    // Set stage
...
int IndexWriter::add_symbols(const std::unordered_set<std::string> & sym){
    if(ensure_stage(IDX_STAGE_SYMBOLS) == -1)
...
int IndexWriter::add_series(uint64_t ref, const label::Labels & l, const std::deque<std::shared_ptr<chunk::ChunkMeta> > & chunks){
    if(ensure_stage(IDX_STAGE_SERIES) == -1)
        return INVALID_STAGE;
    ...
    if(chunks.size() > 0){
        ...
        buf2.put_unsigned_variant(static_cast<uint64_t>(chunks[0]->max_time - chunks[0]->min_time));
        buf2.put_unsigned_variant(chunks[0]->ref);

        for(int i = 1; i < chunks.size(); i ++){
            // LOG_INFO << chunks[i]->min_time - last_t;
            buf2.put_unsigned_variant(static_cast<uint64_t>(chunks[i]->min_time - last_t));
            // LOG_INFO << chunks[i]->max_time - chunks[i]->min_time;
            buf2.put_unsigned_variant(static_cast<uint64_t>(chunks[i]->max_time - chunks[i]->min_time));
...
//
// Will sort values
int IndexWriter::write_label_index(const std::deque<std::string> & names, const std::deque<std::string> & values){
    if(values.size() % names.size() != 0){
        LOG_ERROR << "Invalid length for names and values";
        return INVALID_LENGTH;
    }
    if(ensure_stage(IDX_STAGE_LABEL_INDEX) == -1)
        return INVALID_STAGE;
...
// └─────────────────────────────────────────┘
// Align to 4 bytes
int IndexWriter::write_postings(const std::string & name, const std::string & value, const PostingsInterface * values){
    if(ensure_stage(IDX_STAGE_POSTINGS) == -1)
        return INVALID_STAGE;
