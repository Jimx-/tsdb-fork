...
// Like slice in Go.
std::unique_ptr<PostingsInterface> MemPostings::get(const std::string & label, const std::string & value){
    {
        ...
        if(v_map != m.end()){
            ...
            if(v_list != v_map->second.end())
                return std::unique_ptr<PostingsInterface>(new ListPostings(&(v_list->second)));
        }
    }
    return nullptr;
}
...
void MemPostings::iter(const boost::function<void (const label::Label &, const ListPostings &)> & f){
    base::RWLockGuard mutex(mutex_, 0);
    for(auto const& v_map: m){
        for(auto const& v_list: v_map.second){
            f(label::Label(v_map.first, v_list.first), ListPostings(&(v_list.second)));
        }
    }
}
