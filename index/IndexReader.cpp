...
IndexReader::IndexReader(const std::string & filename): err_(false){
    std::shared_ptr<tsdbutil::ByteSlice> temp = std::shared_ptr<tsdbutil::ByteSlice>(new tsdbutil::MMapSlice(filename));
    if(!validate(temp)){
        LOG_ERROR << "Fail to create IndexReader, invalid ByteSlice";
        err_ = true;
        return;
...
void IndexReader::init(){
    ...
    if(!read_offset_table(toc_pair.first.label_indices_table, true)){
        LOG_ERROR << "Fail to create IndexReader, error reading label indices table";
        b.reset();
        err_ = true;
        return;
    }
...
void IndexReader::get_all_postings(){
    ...
    for(auto const& p_table: postings_table){
        for(auto const& p_pair: p_table.second){
            std::pair<std::unique_ptr<PostingsInterface>, bool> p = postings(p_table.first, p_pair.first);
            while(p.first->next())
                all.insert(p.first->at());
        }
    }
...
bool IndexReader::read_symbols(const TOC & toc){
    std::pair<const uint8_t *, int> symbols_begin = b->range(toc.symbols, toc.symbols + 4);
    if(symbols_begin.second != 4)
        return false;
...
bool IndexReader::read_offset_table(uint64_t offset, bool label_index){
    ...
    if(label_index){
        ...
        for(int i = 0; i < num_entries; i ++){
            uint64_t temp_num = dec_buf.get_unsigned_variant();     // Normally temp_num = 1
...
std::deque<std::string> IndexReader::label_names(){
    auto cmp = [](std::string a, std::string b) { return a.compare(b); };
    ...
    for(auto const& p: label_indices_table){
        if(p.first.length() == 0)
            continue;
        boost::tokenizer<boost::char_separator<char> > label_names_tokens(p.first, tokenSep);
...
// └─────────────────────────────────────────────────┘
// Align to 4 bytes
std::unique_ptr<tsdbutil::StringTuplesInterface> IndexReader::label_values(const std::initializer_list<std::string> & names){
    if(!b)
        return nullptr;
    ...
    ++ it;
    while(it != names.end()){
        label_name += LABEL_NAME_SEPARATOR + std::string((*it).data(), (*it).size());
        ++ it;
    }
    ...
    return std::unique_ptr<tsdbutil::StringTuplesInterface>(new tsdbutil::SerializedStringTuples(start + 12, tuple_size, len - 8, boost::bind(&IndexReader::lookup_symbols, this, _1)));
}
...
std::pair<std::unique_ptr<PostingsInterface>, bool> IndexReader::postings(const std::string & name, const std::string & value){
    if(!b)
        return {std::unique_ptr<PostingsInterface>(new EmptyPostings()), false};

    if(name == label::ALL_POSTINGS_KEYS.label && value == label::ALL_POSTINGS_KEYS.value)
        return {std::unique_ptr<PostingsInterface>(new ListPostings(all_postings)), true};
    ...
    if(postings_table[name].find(value) == postings_table[name].end()){
        LOG_ERROR << "Fail to read postings, label value:" << value << " not existed";
        return {std::unique_ptr<PostingsInterface>(new EmptyPostings()), false}; 
    }
    ...
    uint32_t len = base::get_uint32_big_endian(start);
    // LOG_INFO << len;
    return {std::unique_ptr<PostingsInterface>(new Uint32BEPostings(start + 8, len - 4)), true};
}
...
bool IndexReader::series(uint64_t ref, label::Labels & lset, std::deque<std::shared_ptr<chunk::ChunkMeta> > & chunks){
    if(!b)
        return false;
    ...
    int decoded;
    uint64_t len = base::decode_unsigned_varint(start, decoded, base::MAX_VARINT_LEN_64);
    ...
    chunks.push_back(std::shared_ptr<chunk::ChunkMeta>(new chunk::ChunkMeta(static_cast<uint64_t>(last_ref), last_t, static_cast<int64_t>(delta_t) + last_t)));
...
std::unique_ptr<index::PostingsInterface> IndexReader::sorted_postings(std::unique_ptr<index::PostingsInterface> && p){
    return std::move(p);
}

bool IndexReader::error(){
    return err_;
}
...
uint64_t symbol_table_size(const IndexReader & indexr){
    uint64_t size = 0;
    for(const std::string & s: indexr.symbols_deque())
        size += s.length() + 8;
    return size;
}
