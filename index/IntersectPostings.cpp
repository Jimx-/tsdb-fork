...
IntersectPostings::IntersectPostings(std::deque<std::shared_ptr<PostingsInterface> > & p_s): mode(false){
    for(auto &i: p_s)
        this->p_s.emplace_back(std::move(i));
}

IntersectPostings::IntersectPostings(std::deque<std::unique_ptr<PostingsInterface> > & p_u): mode(true){
    for(auto &i: p_u)
        this->p_u.emplace_back(std::move(i));
}
...
std::shared_ptr<PostingsInterface> intersect_s(std::deque<std::shared_ptr<PostingsInterface> > & list){
    if(list.size() == 0)
        ...
        return std::shared_ptr<PostingsInterface>(std::move(*(list.begin())));
    return std::shared_ptr<PostingsInterface>(new IntersectPostings(list));
}
...
std::unique_ptr<PostingsInterface> intersect_u(std::deque<std::unique_ptr<PostingsInterface> > & list){
    if(list.size() == 0)
        ...
        return std::unique_ptr<PostingsInterface>(std::move(*(list.begin())));
    return std::unique_ptr<PostingsInterface>(new IntersectPostings(list));
}
