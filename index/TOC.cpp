...
const int INDEX_TOC_LEN = 2 * 8 + 4;       // 6 fields + crc32
...
std::string toc_portion_string(const TOC& toc, uint64_t pos)
{
    char temp[100];
    std::sprintf(temp, "%.4f",
                 static_cast<double>(toc.label_indices_table - toc.series) /
                     static_cast<double>(pos));
    std::string s = " series:" + std::string(temp);
    std::sprintf(temp, "%.4f",
                 static_cast<double>(pos - toc.label_indices_table) /
                     static_cast<double>(pos));
...
std::string group_toc_portion_string(const GroupTOC& toc, uint64_t pos)
{
    char temp[100];
    std::sprintf(temp, "%.4f",
                 static_cast<double>(toc.series - toc.symbols) /
                     static_cast<double>(pos));
