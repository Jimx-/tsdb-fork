...
const int INDEX_TOC_LEN = 6 * 8 + 4;        // 6 fields + crc32
const int INDEX_GROUP_TOC_LEN = 8 * 8 + 4;  // 8 fields + crc32

// return false if error
std::pair<TOC, bool> toc_from_ByteSlice(const tsdbutil::ByteSlice * bs){
    if(bs->len() < INDEX_TOC_LEN)
        return std::make_pair<TOC, bool>(TOC(), false);
    std::pair<const uint8_t *, int> b = bs->range(bs->len() - INDEX_TOC_LEN, bs->len());
...
std::pair<GroupTOC, bool> group_toc_from_ByteSlice(const tsdbutil::ByteSlice * bs){
    ...
    std::pair<const uint8_t *, int> b = bs->range(bs->len() - INDEX_GROUP_TOC_LEN, bs->len());
...
std::string toc_string(const TOC & toc){
    std::string s = "symbols:" + std::to_string(toc.symbols) 
...
std::string toc_portion_string(const TOC & toc, uint64_t pos){
    char temp[100];
    std::sprintf(temp, "%.4f", static_cast<double>(toc.series - toc.symbols) / static_cast<double>(pos));
    std::string s = "symbols:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.label_indices - toc.series) / static_cast<double>(pos));
    s += " series:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.postings - toc.label_indices) / static_cast<double>(pos));
    s += " label_indices:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.label_indices_table - toc.postings) / static_cast<double>(pos));
    s += " postings:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.postings_table - toc.label_indices_table) / static_cast<double>(pos));
    s += " label_indices_table:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(pos - toc.postings_table) / static_cast<double>(pos));
...
std::string group_toc_portion_string(const GroupTOC & toc, uint64_t pos){
    ...
    std::sprintf(temp, "%.4f", static_cast<double>(toc.postings - toc.label_indices) / static_cast<double>(pos));
    s += " label_indices:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.group_postings - toc.postings) / static_cast<double>(pos));
    s += " postings:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.label_indices_table - toc.group_postings) / static_cast<double>(pos));
    s += " group postings:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.postings_table - toc.label_indices_table) / static_cast<double>(pos));
    s += " label_indices_table:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(toc.group_postings_table - toc.postings_table) / static_cast<double>(pos));
    s += " postings_table:" + std::string(temp);
    std::sprintf(temp, "%.4f", static_cast<double>(pos - toc.group_postings_table) / static_cast<double>(pos));
