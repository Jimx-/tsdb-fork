...
class IndexWriter: public block::IndexWriterInterface{
    private:
        ...
        tsdbutil::CacheVector<uint32_t> uint32_cache;                 // For sorting when writing postings list
    ...
    public:
        ...
        //
        // Will sort values
        int write_label_index(const std::deque<std::string> & names, const std::deque<std::string> & values);
        ...
        // └─────────────────────────────────────────┘
        // Align to 4 bytes
        int write_postings(const std::string & name, const std::string & value, const PostingsInterface * values);
