...
class Head: public block::BlockInterface{
    public:
        ...
        base::AtomicInt64 valid_time; // Shouldn't be lower than the max_time of the last persisted block
        ...
        std::unordered_map<std::string, std::unordered_set<std::string>> label_values;  // label name to possible values
        ...
        Head(int64_t chunk_range, std::unique_ptr<wal::WAL> && wal, const std::shared_ptr<base::ThreadPool> & pool_);
        ...
        void process_wal_samples(const std::deque<tsdbutil::RefSample> & samples, base::AtomicUInt64 * unknown_refs, base::WaitGroup * wg);
        ...
        std::pair<std::shared_ptr<MemSeries>, bool> get_or_create(uint64_t hash, const label::Labels & lset);

        std::pair<std::shared_ptr<MemSeries>, bool> get_or_create_with_id(uint64_t id, uint64_t hash, const label::Labels & lset);
        ...
        error::Error chunk_rewrite(uint64_t ref, const tombstone::Intervals & dranges);
