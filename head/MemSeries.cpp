...
bool MemIterator::error() const{
    return iterator->error();
}

MemSeries::MemSeries(const label::Labels & labels, uint64_t id, int64_t chunk_range): 
    mutex_(), 
    ref(id), 
    labels(labels),
...
std::pair<bool ,bool> MemSeries::append(int64_t timestamp, double value){
    ...
    if(h->max_time >= timestamp)
        return {false, chunk_created};

    // If we reach 25% of a chunk's desired sample count, set a definitive time
    // at which to start the next chunk.
    // At latest it must happen at the timestamp set when the chunk was cut.
    // If it already reaches 1/4, then we may cut it earlier to avoid the too long chunk
...
std::shared_ptr<MemChunk> MemSeries::cut(int64_t timestamp){
    chunks.emplace_back(
        new MemChunk(
            // NOTICE(Alec), remember to add ref here.
            ref,
    ...
    //
    // NOTE(Alec), this can make the chunk not cross the boundary of chunk_range.
    // And because the HeadChunkReader of RangeHead only checks ChunkMeta::overlap_closed()
    // , which will not exclude the crossing-boundary chunks.
    // Thus, the LeveledCompactor::write() will not fail because of crossing boundary.
...
int64_t MemSeries::chunk_id(int pos){
    return first_chunk + pos;
}
...
std::unique_ptr<chunk::ChunkIteratorInterface> MemSeries::iterator(int id){
    ...
    if(!c)
        return nullptr;
    if(id - first_chunk < chunks.size())
        return c->chunk->iterator();

    // Serve the last 4 samples for the last chunk from the sample buffer
    // as their compressed bytes may be mutated by added samples.
    return std::unique_ptr<chunk::ChunkIteratorInterface>(new MemIterator(c, sample_buf));
}
