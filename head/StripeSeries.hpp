...
class StripeSeries{
    public:
        std::vector<std::unordered_map<uint64_t, std::shared_ptr<MemSeries>>> series; // Index by mod series ref.
        std::vector<SeriesHashmap> hashes; // Index by mod hash.
        std::vector<base::PadRWMutexLock> locks; // To align cache line (multiples of 64 bytes)
        ...
        std::shared_ptr<MemSeries> get_by_hash(uint64_t hash, const label::Labels & lset);

        // Return <MemSeries, if the series being set>.
        std::pair<std::shared_ptr<MemSeries>, bool> get_or_set(uint64_t hash, const std::shared_ptr<MemSeries> & s);
};
