...
MergedSeriesSet::MergedSeriesSet(const std::shared_ptr<SeriesSets> & ss): ss(ss), series(new Series()), err_(false){
...
bool MergedSeriesSet::next_helper() const{
    ...
    for(int i = 1; i < ss->size(); i ++){
        int cmp = label::lbs_compare(ss->at(i)->at()->labels(), ss->at(0)->at()->labels());
        if(cmp < 0){
            // series->clear();
            id.clear();
...
std::shared_ptr<SeriesInterface> MergedSeriesSet::at(){
    if(id.empty())
        ...
        // LOG_INFO << "Create ChainSeries(same lset in several SeriesSetInterface), num of SeriesInterface: " << series->size();
        return std::shared_ptr<SeriesInterface>(new ChainSeries(series));
    }
}

bool MergedSeriesSet::error() const{
    return err_;
}
