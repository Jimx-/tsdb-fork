...
// Similar to BaseChunkSeriesSet, but it has two extra fields: 1.min_time 2.max_time,
// which are used for filtering the chunks not in time range.
// NOTE(Alec), PopulatedChunkSeriesSet coarse-grained filters the chunks using min_time and max_time.
PopulatedChunkSeriesSet::PopulatedChunkSeriesSet(const std::shared_ptr<ChunkSeriesSetInterface> & set, 
    const std::shared_ptr<block::ChunkReaderInterface> & chunkr,
    int64_t min_time,
    int64_t max_time): set(set), chunkr(chunkr), min_time(min_time), max_time(max_time), cm(new ChunkSeriesMeta()){}
...
bool PopulatedChunkSeriesSet::next() const{
    while(set->next()){
        ...
        while(!cm->chunks.empty()){
            if(cm->chunks[0]->max_time >= min_time)
                break;
            cm->chunks.pop_front();
        }
...
bool PopulatedChunkSeriesSet::error() const{
    return err_;
}
