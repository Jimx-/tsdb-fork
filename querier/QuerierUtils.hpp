...
private:
    std::vector<std::shared_ptr<ChunkSeriesSetInterface>> css;
...
public:
    ...
    void clear();
    void next();
    void next(const std::vector<int>& id);
...
//         GroupChunkSeriesSets(const
//         std::deque<std::shared_ptr<GroupChunkSeriesSetInterface> > & css);

//         void push_back(const std::shared_ptr<GroupChunkSeriesSetInterface> &
//         s); void clear(); void next(); void next(const std::deque<int> & id);
//         std::shared_ptr<GroupChunkSeriesSetInterface> & operator[](int i);
//         std::shared_ptr<GroupChunkSeriesSetInterface> operator[](int i)
//         const; std::shared_ptr<GroupChunkSeriesSetInterface> & at(int i); int
//         size(); bool empty();
// };
