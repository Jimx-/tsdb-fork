...
class ChunkSeriesMeta{
    public:
        ...
        void sort_by_min_time(){
            std::sort(chunks.begin(), chunks.end(), [](const std::shared_ptr<chunk::ChunkMeta> & lhs, const std::shared_ptr<chunk::ChunkMeta> & rhs){
...
class GroupChunkSeriesMeta{
    public:
        ...
        void sort_by_min_time(){
            for(int i = 0; i < chunks.size(); ++ i){
                std::sort(chunks[i].begin(), chunks[i].end(), [](const std::shared_ptr<chunk::ChunkMeta> & lhs, const std::shared_ptr<chunk::ChunkMeta> & rhs){
                    return lhs->min_time < rhs->min_time;
                });
            }
        }
