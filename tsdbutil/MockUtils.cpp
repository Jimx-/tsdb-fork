...
void print_seriessamples(const SeriesSamples & s){
    ...
    for(int i = 0; i < s.chunks.size(); ++ i){
        ...
        for(int j = 0; j < s.chunks[i].size(); ++ j)
            str += "<" + std::to_string(s.chunks[i][j].t) + "," + std::to_string(s.chunks[i][j].v) + ">,";
        str += "],";
    }
    LOG_DEBUG << str;
}
...
std::tuple<std::shared_ptr<block::IndexReaderInterface>, std::shared_ptr<block::ChunkReaderInterface>, int64_t, int64_t> create_idx_chk_readers(std::deque<SeriesSamples> & tc){
    std::sort(tc.begin(), tc.end(), [](const SeriesSamples & lhs, const SeriesSamples & rhs){
        return label::lbs_compare(label::lbs_from_map(lhs.lset), label::lbs_from_map(rhs.lset)) < 0;
    });
    ...
    for(int i = 0; i < tc.size(); ++ i){
        ...
        for(auto const& chk: tc[i].chunks){
            if(chk.front().t < block_mint)
                block_mint = chk.front().t;
            if(chk.back().t > block_maxt)
                block_maxt = chk.back().t;

            s.chunks.push_back(std::shared_ptr<chunk::ChunkMeta>(new chunk::ChunkMeta(ref, chk.front().t, chk.back().t)));
        ...
        for(auto const& l: s.lset)
            ld[l.label].insert(l.value);
    }
    ...
    postings.iter([ir](const label::Label & l, const index::ListPostings & p){
        while(p.next())
            ir->postings_[l].push_back(p.at());
    });
    return std::make_tuple(std::shared_ptr<block::IndexReaderInterface>(ir), std::shared_ptr<block::ChunkReaderInterface>(cr), block_mint, block_maxt);
}
