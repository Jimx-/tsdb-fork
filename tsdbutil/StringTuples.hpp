...
class StringTuples{
    public:
        ...
        StringTuples(){}
        StringTuples(const std::deque<std::string> & entries, int length): entries(entries), length(length){}

        int len(){
            return entries.size() / length;
        }
        ...
        std::deque<std::string> at(int i){
            ...
            return std::deque<std::string>(entries.begin() + i * length, entries.begin() + (i + 1) * length);
        }
