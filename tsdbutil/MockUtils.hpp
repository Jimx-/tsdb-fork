...
class SeriesSamples{
    ...
    public:
        ...
        SeriesSamples(const std::unordered_map<std::string, std::string> & lset, const std::deque<std::deque<Sample>> & chunks): lset(lset), chunks(chunks){}
        SeriesSamples(const std::unordered_map<std::string, std::string> & lset): lset(lset){}

        bool operator==(const SeriesSamples & s) const{
            if(label::lbs_compare(label::lbs_from_map(lset), label::lbs_from_map(s.lset)) != 0)
                return false;
            if(chunks.size() != s.chunks.size())
                return false;
            for(int i = 0; i < chunks.size(); ++ i){
                if(chunks[i].size() != s.chunks[i].size())
                    return false;
                for(int j = 0; j < chunks[i].size(); ++ j){
                    if(chunks[i][j] != s.chunks[i][j])
                        return false;
                }
            }
            return true;
...
class MockIndexReader: public block::IndexReaderInterface{
    public:
        ...
        std::unordered_map<label::Label, std::deque<uint64_t>, label::LabelHasher> postings_;
        ...
        std::unique_ptr<tsdbutil::StringTuplesInterface> label_values(const std::initializer_list<std::string> & names){
            if(names.size() != 1)
                return nullptr;
            return std::unique_ptr<tsdbutil::StringTuplesInterface>(new tsdbutil::ListStringTuples(std::vector<std::string>(label_index[*names.begin()].begin(), label_index[*names.begin()].end())));
        }
        std::pair<std::unique_ptr<index::PostingsInterface>, bool> postings(const std::string & name, const std::string & value){
            ...
            return {std::unique_ptr<index::PostingsInterface>(new index::ListPostings(postings_[label::Label(name, value)])), true};
        }
        bool series(uint64_t ref, label::Labels & lset, std::deque<std::shared_ptr<chunk::ChunkMeta> > & chunks){
            if(series_.find(ref) == series_.end())
                return false;
            for(auto const& l: series_[ref].lset)
                lset.push_back(l);
            for(auto const& c: series_[ref].chunks)
                chunks.push_back(c);
            return true;
        }
        std::deque<std::string> label_names(){
            std::deque<std::string> names;
            for(auto const& p: label_index)
        ...
        std::unique_ptr<index::PostingsInterface> sorted_postings(std::unique_ptr<index::PostingsInterface> && p){
            ...
            std::sort(ep.begin(), ep.end(), [this](int i, int j){
                return label::lbs_compare(this->series_[i].lset, this->series_[j].lset) < 0;
            });
...
class MockIndexWriter: public block::IndexWriterInterface{
    public:
        ...
        int add_series(uint64_t ref, const label::Labels & l, const std::deque<std::shared_ptr<chunk::ChunkMeta> > & chunks){
            int i = -1;
        ...
        int write_label_index(const std::deque<std::string> & names, const std::deque<std::string> & values){ return 0; }
        int write_postings(const std::string & name, const std::string & value, const index::PostingsInterface * values){ return 0; }
};

class NopChunkWriter: public block::ChunkWriterInterface{
    public:
        void write_chunks(const std::deque<std::shared_ptr<chunk::ChunkMeta> > & chunks){}
        void close(){}
};
...
class MockBlockReader: public block::BlockInterface{
    public:
        ...
        MockBlockReader(): min_time(std::numeric_limits<int64_t>::max()), max_time(std::numeric_limits<int64_t>::min()){}
        MockBlockReader(const std::shared_ptr<block::IndexReaderInterface> & ir, const std::shared_ptr<block::ChunkReaderInterface> & cr, int64_t min_time, int64_t max_time): ir(ir), cr(cr), min_time(min_time), max_time(max_time){}

        // index returns an IndexReader over the block's data, succeed or not.
        std::pair<std::shared_ptr<block::IndexReaderInterface>, bool> index()const{ return {ir, true}; }

        // chunks returns a ChunkReader over the block's data, succeed or not.
        std::pair<std::shared_ptr<block::ChunkReaderInterface>, bool> chunks()const{ return {cr, true}; }

        // tombstones returns a TombstoneReader over the block's deleted data, succeed or not.
        std::pair<std::shared_ptr<tombstone::TombstoneReaderInterface>, bool> tombstones()const{ return {std::shared_ptr<tombstone::TombstoneReaderInterface>(new tombstone::MemTombstones()), true}; }
...
std::tuple<std::shared_ptr<block::IndexReaderInterface>, std::shared_ptr<block::ChunkReaderInterface>, int64_t, int64_t> create_idx_chk_readers(std::deque<SeriesSamples> & tc);
