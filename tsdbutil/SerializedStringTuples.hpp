...
class SerializedStringTuples: public StringTuplesInterface{
    public:
        ...
        SerializedStringTuples(const uint8_t * b, uint32_t tuple_size, uint32_t size, const Lookup & lookup): b(b), size(size), tuple_size(tuple_size), len_(size / (tuple_size * 4)), lookup(lookup){}
