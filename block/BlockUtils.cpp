...
std::pair<BlockMeta, bool> read_block_meta(const std::string & dir){
    boost::filesystem::path p = boost::filesystem::path(dir) / boost::filesystem::path(META_FILE_NAME);
    ...
    if(d.HasMember("compaction")){
        ...
        if(d["compaction"].HasMember("sources")){
            ...
            for(rapidjson::SizeType i = 0; i < d["compaction"]["sources"].Size(); i ++){
                if(!d["compaction"]["sources"][i].IsString()){
                    return {BlockMeta(), false};
                }
                meta.compaction.sources.push_back(ulid::Unmarshal(d["compaction"]["sources"][i].GetString()));
            }
        }
        ...
        if(d["compaction"].HasMember("parents")){
            ...
            for(rapidjson::SizeType i = 0; i < d["compaction"]["parents"].Size(); i ++){
                BlockDesc temp;
                if(d["compaction"]["parents"][i].HasMember("ulid")){
                    if(!d["compaction"]["parents"][i]["ulid"].IsString()){
                        return {BlockMeta(), false};
                    }
                    temp.ulid_ = ulid::Unmarshal(d["compaction"]["parents"][i]["ulid"].GetString());
                }
...
bool write_block_meta(const std::string & dir, const BlockMeta & meta){
    ...
    rapidjson::Document d;
    d.SetObject();
    rapidjson::Value stats(rapidjson::kObjectType), compaction(rapidjson::kObjectType);

    d.AddMember("ulid", rapidjson::Value(ulid::Marshal(meta.ulid_).data(), d.GetAllocator()).Move(), d.GetAllocator());
    ...
    compaction.AddMember("deletable", meta.compaction.deletable, d.GetAllocator());
    if(meta.compaction.sources.size() > 0){
        rapidjson::Value sources(rapidjson::kArrayType);
        for(auto const & i: meta.compaction.sources)
            sources.PushBack(rapidjson::Value(ulid::Marshal(i).data(), d.GetAllocator()).Move(), d.GetAllocator());
    ...
    if(meta.compaction.parents.size() > 0){
        ...
        for(auto const & i: meta.compaction.parents){
            ...
            desc.AddMember("ulid", rapidjson::Value(ulid::Marshal(i.ulid_).data(), d.GetAllocator()).Move(), d.GetAllocator());
