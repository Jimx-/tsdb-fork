...
TimeRangeOverlaps overlapping_blocks(const block::BlockMetas & bms){
    ...
    // We have here blocks sorted by min_time. We iterate over each block and treat its minTime as our "current" timestamp.
    for(int i = 1; i < bms.size(); ++ i){
        ...
        if(!chained && !overlapping.empty()){
            ...
            for(int j: overlapping)
                overlaps.map_[t].push_back(bms.at(j));
            overlapping.clear();
        }
        if(bms[i].max_time > maxt)
            maxt = bms[i].max_time;
    }
