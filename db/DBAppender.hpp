...
class DBAppender: public AppenderInterface{
    ...
    public:
        DBAppender(std::unique_ptr<db::AppenderInterface> && app, db::DB * db): app(std::move(app)), db(db){}
        ...
        error::Error commit(){
            ...
            if(db->head()->MaxTime() - db->head()->MinTime() > db->head()->chunk_range / 2 * 3){
                db->compact_channel()->send(0);
                // LOG_DEBUG << db->head()->MinTime() << " " << db->head()->MaxTime() << " " << (db->head()->MaxTime() - db->head()->MinTime() > db->head()->chunk_range / 2 * 3);
            }
