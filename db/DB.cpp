...
DB::DB(const std::string & dir_, const Options & options): 
    dir_(dir_),
    opts(options),
    compactc(new base::Channel<char>()),
    donec(new base::Channel<char>()),
    stopc(new base::Channel<char>()),
    compact_cancel(new base::Channel<char>()),
    auto_compact(true),
    pool_(new base::ThreadPool("DB ThreadPool"))
{
    ...
    compactor = std::unique_ptr<compact::CompactorInterface>(new compact::LeveledCompactor(opts.block_ranges, compact_cancel));
    if(compactor->error()){
    ...
    if(opts.wal_segment_size >= 0){
        if(opts.wal_segment_size > 0)
            wal = std::unique_ptr<wal::WAL>(new wal::WAL(tsdbutil::filepath_join(dir_, "wal"), pool_, opts.wal_segment_size));
        else
            wal = std::unique_ptr<wal::WAL>(new wal::WAL(tsdbutil::filepath_join(dir_, "wal"), pool_));
        if(wal->error()){
    ...
    head_ = std::shared_ptr<head::Head>(new head::Head(opts.block_ranges[0], std::move(wal), pool_));
    if(head_->error()){
    ...
    if(!b.empty())
        min_valid_time = b.back()->meta().max_time;
    err = head_->init(min_valid_time);
    if(err)
        err_.set(error::wrap(err, "error head::init"));
...
std::unique_ptr<db::AppenderInterface> DB::appender(){
    return std::unique_ptr<db::AppenderInterface>(new DBAppender(std::move(head_->appender()), this));
}

std::pair<std::unique_ptr<querier::QuerierInterface>, error::Error> DB::querier(int64_t mint, int64_t maxt){
    std::deque<std::shared_ptr<block::BlockInterface>> bs; // block::BlockInterface for constructing querier::BlockQuerier.
    ...
    if(maxt >= head_->MinTime())
        bs.push_back(std::shared_ptr<block::BlockInterface>(new head::RangeHead(head_, mint, maxt)));

    std::deque<std::shared_ptr<querier::QuerierInterface> > queriers;
    for(auto const& b: bs){
        std::shared_ptr<querier::QuerierInterface> q(new querier::BlockQuerier(b, mint, maxt));
        if(!q->error()){
            queriers.push_back(q);
            continue;
        }
        // If we fail, all previously opened queriers must be closed.
        // clear() will call the destructor of indexr, chunkr, tombstones. Thus, close the BlockQuerier and BlockInterface.
        queriers.clear();
        return {nullptr, error::wrap(q->error(), "open querier for block " + b->dir())};
    }

    if(!overlapping_blocks(bms).empty()){
        // TODO(Alec), add vertical querier.
        return {nullptr, error::Error("cannot get querier on overlapping blocks")};
    }

    return {std::unique_ptr<querier::QuerierInterface>(new querier::Querier(queriers)), error::Error()};
}

std::deque<std::shared_ptr<block::BlockInterface>> DB::blocks(){
    base::RWLockGuard lock(mutex_, 0);
    std::deque<std::shared_ptr<block::BlockInterface>> r(blocks_.begin(), blocks_.end());
    return r;
}
...
std::shared_ptr<block::BlockInterface> DB::get_block(const ulid::ULID & ulid){
    base::RWLockGuard lock(mutex_, 0);
    for(auto & b: blocks_){
        if(ulid::CompareULIDs(b->meta().ulid_, ulid) == 0)
            return b;
    }
    return nullptr;
}

std::unordered_map<ulid::ULID, error::Error> DB::open_blocks(std::deque<std::shared_ptr<block::BlockInterface>> & blocks){
    std::deque<std::string> dirs = block_dirs(dir_);

    std::unordered_map<ulid::ULID, error::Error> corrupted;
    for(auto const& dir: dirs){
        std::pair<block::BlockMeta, bool> meta_pair = block::read_block_meta(dir);
        if(!meta_pair.second){
        ...
        std::shared_ptr<block::BlockInterface> b = get_block(meta_pair.first.ulid_);
        if(b == nullptr){
...
std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> DB::deletable_blocks(std::deque<std::shared_ptr<block::BlockInterface>> & blocks){
    std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> deletable;

    // Sort the blocks by time - newest to oldest (largest to smallest timestamp).
    // This ensures that the retentions will remove the oldest  blocks.
    std::sort(blocks.begin(), blocks.end(), [](const std::shared_ptr<block::BlockInterface> & lhs, const std::shared_ptr<block::BlockInterface> & rhs){
...
// The blocks passed in are sorted by time - newest to oldest (largest to smallest timestamp).
std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> DB::beyond_time_retention(std::deque<std::shared_ptr<block::BlockInterface>> & blocks){
    std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> deletable;

    // Time retention is disabled or no blocks to work with.
    if(blocks.empty() || opts.retention_duration == 0)
        return deletable;

    for(int i = 0; i < blocks.size(); ++ i){
        // The difference between the first block and this block is larger than
        // the retention period so any blocks after that are added as deleteable.
        if(i > 0 && blocks[0]->meta().max_time - blocks[i]->meta().max_time > static_cast<int64_t>(opts.retention_duration)){
...
// The blocks passed in are sorted by time - newest to oldest (largest to smallest timestamp).
std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> DB::beyond_size_retention(std::deque<std::shared_ptr<block::BlockInterface>> & blocks){
    std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> deletable;

    // Size retention is disabled or no blocks to work with.
    if(blocks.empty() || opts.max_bytes <= 0)
        return deletable;
...
void DB::delete_blocks(const std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> & deletable){
    for(auto const& p: deletable){
        if(p.second){
            // This is a blocking function.
            p.second->close();
        }
        boost::filesystem::remove_all(tsdbutil::filepath_join(dir_, ulid::Marshal(p.first)));
    }
}

// validate_block_sequence returns error if given block meta files indicate that some blocks overlaps within sequence.
// blocks are sorted by min_time.
error::Error DB::validate_block_sequence(const std::deque<std::shared_ptr<block::BlockInterface>> & blocks){
    if(blocks.size() <= 1)
        return error::Error();
...
error::Error DB::reload(){
    ...
    std::unordered_map<ulid::ULID, error::Error> corrupted = open_blocks(loadable);

    std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> deletable = deletable_blocks(loadable);
    ...
    if(!corrupted.empty()){
        ...
        for(auto const& b: loadable){
            std::shared_ptr<block::BlockInterface> block = get_block(b->meta().ulid_);
            if(block)
                block->close();
        }
        std::string s;
        ...
        return error::Error("unexpected corrupted blocks: " + s.substr(0, s.length() - 2));
    }
    ...
    std::sort(loadable.begin(), loadable.end(), [](const std::shared_ptr<block::BlockInterface> & lhs, const std::shared_ptr<block::BlockInterface> & rhs){
    ...
    if(!opts.allow_overlapping_blocks){
        ...
        if(err)
            return error::wrap(err, "invalid block sequence(overlapping blocks)");
    }
    ...
    TimeRangeOverlaps overlaps = overlapping_blocks(bms);
    if(!overlaps.empty())
        LOG_WARN << "msg=\"overlapping blocks found during reload\" detail=" <<  overlaps.str();
...
// which will also trigger blocks to be deleted that fall out of the retention window.
...
error::Error DB::compact(){
    ...
    while(true){
        ...
        std::pair<ulid::ULID, error::Error> ulid_pair = compactor->write(dir_, h, mint, maxt, nullptr);
        if(ulid_pair.second)
        ...
        if(err){
            try{
                boost::filesystem::remove_all(tsdbutil::filepath_join(dir_, ulid::Marshal(ulid_pair.first)));
            }
            catch(const boost::filesystem::filesystem_error & e){
                return error::wrap(err, "delete persisted head block after failed db reload: " + ulid::Marshal(ulid_pair.first));
            }
    ...
    while(true){
        std::pair<std::deque<std::string>, error::Error> plan = compactor->plan(dir_);
        if(plan.second)
            return error::wrap(plan.second, "plan compaction");
        if(plan.first.empty())
            break;
        ...
        std::pair<ulid::ULID, error::Error> ulid_pair = compactor->compact(dir_, plan.first, std::shared_ptr<block::Blocks>(new block::Blocks(blocks_)));
        if(ulid_pair.second){
...
void DB::run(){
    int backoff = 0;
    std::shared_ptr<base::Channel<char>> backoff_chan(new base::Channel<char>());
    while(true){
        int select = -1;
        pool_->run(boost::bind(&backoff_timing, backoff_chan, backoff));
        while((select = base::channel_select<char>({stopc, backoff_chan})) == -1){}
        if(select == 0){
            backoff_chan->notify();
            break;
        }
        ...
        if(select == 0){
            ...
            // NOTE(Alec), the main thread can already created a compact signal before backoff_timing() ends,
            // so need to call notify() here to wake up asleep backoff_timing thread.
            compactc->notify();
            if(auto_compact){
                error::Error err = compact();
                if(err){
                    LOG_ERROR << "msg=\"compaction failed\" err=" << err.error();
                    backoff = exponential(backoff, 1, 60); // backoff time between 1s and 1m.
                    if(!compact_cancel->empty())
                        break;
                }
                else
                    backoff = 0;
...
void del_helper(std::shared_ptr<block::BlockInterface> b, int64_t mint, int64_t maxt, const std::deque<std::shared_ptr<label::MatcherInterface>> * matchers, base::WaitGroup * wg, error::MultiError * multi_err){
    // LOG_DEBUG << ulid::Marshal(b->meta().ulid_) << " mint:" << mint << " maxt:" << maxt << " matcher:" << (*matchers)[0]->name() << " " << (*matchers)[0]->value();
...
error::Error DB::del(int64_t mint, int64_t maxt, const std::deque<std::shared_ptr<label::MatcherInterface>> & matchers){
    ...
    // On disk blocks.
    for(auto & b: blocks_){
        // LOG_DEBUG << ulid::Marshal(b->meta().ulid_) << " mint:" << b->meta().min_time << " maxt:" << b->meta().max_time;
        if(b->overlap_closed(mint, maxt)){
            wg.add(1);
            pool_->run(boost::bind(&del_helper, b, mint, maxt, &matchers, &wg, &multi_err));
        }
    }
    ...
    if(head_->overlap_closed(mint, maxt)){
        wg.add(1);
        // std::shared_ptr<head::Head> can be converted to std::shared_ptr<block::BlockInterface> automatically.
        pool_->run(boost::bind(&del_helper, head_, mint, maxt, &matchers, &wg, &multi_err));
    }
    wg.wait();
...
error::Error DB::clean_tombstones(){
    ...
    for(auto & b: bs){
        std::pair<ulid::ULID, error::Error> ulid_pair = b->clean_tombstones(dir_, compactor.get());
        if(ulid_pair.second){
    ...
    if(err){
        for(ulid::ULID & uid: new_ulids){
            try{
                boost::filesystem::remove(tsdbutil::filepath_join(dir_, ulid::Marshal(uid)));
            }
            catch(const boost::filesystem::filesystem_error & e){
                LOG_ERROR << "msg=\"failed to delete block after failed `clean_tombstones()`\" dir=" << tsdbutil::filepath_join(dir_, ulid::Marshal(uid)) << " err=" << err.error();
            }
        }
    }
    err = reload();
    if(err)
        return error::wrap(err, "reload blocks in 'clean_tombstones()'");
    return error::Error();
}
