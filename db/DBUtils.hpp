...
std::vector<int64_t> exponential_block_ranges(int64_t min_size, int step, int step_size);
...
class Options{
    public:
        ...
        Options(): wal_segment_size(0), retention_duration(0), max_bytes(0), no_lock_file(false), allow_overlapping_blocks(false){}
        Options(int wal_segment_size, uint64_t retention_duration, int64_t max_bytes, const std::vector<int64_t> & block_ranges, bool no_lock_file, bool allow_overlapping_blocks):
...
class TimeRange{
    public:
        ...
        TimeRange(int64_t min_time, int64_t max_time): min_time(min_time), max_time(max_time){}
...
class TimeRangeOverlaps{
    ...
    public:
        std::unordered_map<TimeRange, std::deque<block::BlockMeta>, TimeRangeHasher> map_;
            ...
            for(auto const& mp: map_){
                ...
                for(auto const& bm: mp.second){
                    group += "<ulid: " + ulid::Marshal(bm.ulid_) 
                        ...
                        + ", maxt: " + std::to_string(bm.max_time) 
                        + ", range: " + std::to_string((bm.max_time - bm.min_time) / 1000) + "s>, ";
                }
                if(!group.empty())
                    group = group.substr(0, group.length() - 2);
                s += "[mint: " + std::to_string(mp.first.min_time)
                    + ", maxt: " + std::to_string(mp.first.max_time)
                    + ", range: " + std::to_string((mp.first.max_time - mp.first.min_time) / 1000) + "s, "
                    + "blocks: " + std::to_string(mp.second.size())
                    + "]: " + group + "\n";
            }
