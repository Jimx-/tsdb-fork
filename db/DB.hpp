...
class DB{
    ...
    public:
        ...
        std::unordered_map<ulid::ULID, error::Error> open_blocks(std::deque<std::shared_ptr<block::BlockInterface>> & blocks_);

        // deletable_blocks sorts the loadable blocks and 
        // returns all blocks past retention policy.
        std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> deletable_blocks(std::deque<std::shared_ptr<block::BlockInterface>> & blocks_);
        std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> beyond_time_retention(std::deque<std::shared_ptr<block::BlockInterface>> & blocks_);
        std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> beyond_size_retention(std::deque<std::shared_ptr<block::BlockInterface>> & blocks_);
        ...
        void delete_blocks(const std::unordered_map<ulid::ULID, std::shared_ptr<block::BlockInterface>> & deletable);
        ...
        error::Error validate_block_sequence(const std::deque<std::shared_ptr<block::BlockInterface>> & blocks_);
        ...
        std::pair<std::unique_ptr<querier::QuerierInterface>, error::Error> querier(int64_t mint, int64_t maxt);
