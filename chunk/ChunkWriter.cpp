...
ChunkWriter::ChunkWriter(const std::string & dir): dir(dir), pos(0), chunk_size(DEFAULT_CHUNK_SIZE){
    ...
    if(!boost::filesystem::create_directories(block_dir)){
        ...
        // throw AlecException("Directory Already Created: " + wal_path.string());
    }
    else{
...
void ChunkWriter::write(const uint8_t * bytes, int size){
    int written = fwrite(reinterpret_cast<const void*>(bytes), 1, size, files.back());
    pos += written;
}

void ChunkWriter::write_chunks(const std::deque<std::shared_ptr<ChunkMeta> > & chunks){
    uint64_t max_len = 0;
    for(auto & ptr: chunks){
    ...
    if(files.empty() || pos > chunk_size || (pos + max_len > chunk_size && max_len <= chunk_size))
        cut();
    ...
    for(auto & chk : chunks){
        ...
        int encoded = base::encode_unsigned_varint(b, static_cast<uint64_t>(chk->chunk->size()));
        write(b, encoded);
        ...
        // Write crc32
        base::put_uint32_big_endian(b, base::GetCrc32(chk->chunk->bytes(), chk->chunk->size()));
        write(b, 4);
    }
}
