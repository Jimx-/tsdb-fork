...
BitStream::BitStream(BitStream & bstream, bool iterator_mode): 
    vector_mode(bstream.vector_mode),
    end(bstream.end)
{
    if(vector_mode){
...
BitStream::BitStream(const std::vector<uint8_t> & stream): stream(stream), head_count(stream.size() >= 1 ? 8 : 0), tail_count(0), vector_mode(true), index(0), end(stream.size()){}

// Read mode & pointer_mode
BitStream::BitStream(const uint8_t * stream_ptr, int size): stream_ptr(stream_ptr), head_count(size >= 1 ? 8 : 0), tail_count(0), vector_mode(false), index(0), end(size){}
...
uint64_t BitStream::read_bits(int num){
    ...
    if(vector_mode){
        ...
        result = (result << num) | ((stream[index] >> (head_count - num)) & ((1 << num) - 1));
        head_count -= num;
    }
    else{
        if(head_count < num){
            result = (result << head_count) | (stream_ptr[index] & ((1 << head_count) - 1));
            num -= head_count;
            ++index;
            if(index >= end){
                throw base::TSDBException("bitstream EOF");
        ...
        result = (result << num) | ((stream_ptr[index] >> (head_count - num)) & ((1 << num) - 1));
        head_count -= num;
    }
    return result;
}
...
uint64_t BitStream::read_bits(int pos, int num){
    ...
    uint64_t result = 0;
    while(num >= 8){
        uint8_t temp = read_byte_helper(index_, head_count_);
        num -= 8;
        result = (result << 8) | (temp);
    }
    if(vector_mode){
        if(head_count_ < num){
            result = (result << head_count_) | (stream[index_] & ((1 << head_count_) - 1));
            num -= head_count_;
            ++index_;
            if(index_ >= end){
                throw base::TSDBException("bitstream EOF");
        ...
        result = (result << num) | ((stream[index_] >> (head_count_ - num)) & ((1 << num) - 1));
        // std::cout << " T " << (stream.front() >> (head_count - num));
        head_count_ -= num;
    }
    else{
        if(head_count_ < num){
            result = (result << head_count_) | (stream_ptr[index_] & ((1 << head_count_) - 1));
            num -= head_count_;
            ++index_;
            if(index_ >= end){
                throw base::TSDBException("bitstream EOF");
        ...
        result = (result << num) | ((stream_ptr[index_] >> (head_count_ - num)) & ((1 << num) - 1));
        // std::cout << " T " << (stream.front() >> (head_count - num));
        head_count_ -= num;
    }
...
std::pair<int64_t, uint8_t> BitStream::read_signed_varint(int pos){
    ...
    return {static_cast<int64_t>( unsigned_value.first & 1 ? ~(unsigned_value.first >> 1) 
                                         :  (unsigned_value.first >> 1) ), unsigned_value.second};
}
...
std::vector<uint8_t> & BitStream::get_stream(){
    return stream;
}
