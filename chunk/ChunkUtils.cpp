...
// #include "chunk/GroupDiskChunk1.hpp"
// #include "chunk/GroupMemoryChunk1.hpp"
...
//     int last = 0;
//     for(int i = 1; i < chunks[0].size(); i ++){
//         if(chunks[0][i]->min_time > gcsm->chunks[0][last]->max_time){
//             for(int j = 0; j < chunks.size(); ++ j)
//                 gcsm->chunks[j].push_back(chunks[j][i]);
//             ++ last;
//             continue;
//         }
//         // Find the overlapping chain.
//         int end = i + 1;
...
//         }
//         // Update the max_time.
//         if(gcsm->chunks[0][last]->max_time < temp_max){
//             for(int j = 0; j < gcsm->chunks.size(); ++ j)
//                 gcsm->chunks[j][last]->max_time = temp_max;
//         }

//         /*
//             For now the min_time and the max_time are set.
//         */

//         // Compress chunnks in [last, end).
//         merge_group_chunks(gcsm, chunks, last, end);
//     }
// }
