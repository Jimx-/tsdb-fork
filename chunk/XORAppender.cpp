...
XORAppender::XORAppender(BitStream & bstream, int64_t timestamp, double value, uint64_t delta_timestamp, uint8_t leading_zero, uint8_t trailing_zero):
    bstream(bstream),
    timestamp(timestamp),
    value(value),
...
void XORAppender::set_leading_zero(uint8_t lz){
    leading_zero = lz;
}

void XORAppender::set_trailing_zero(uint8_t tz){
    trailing_zero = tz;
}

void XORAppender::write_delta_value(double value){
    uint64_t delta_vale = base::encode_double(value) ^ base::encode_double(this->value);
    ...
    if(leading_zero != 0xff && lz >= leading_zero && tz >= trailing_zero){
        // std::cout << "write bits " << 64 - static_cast<int>(leading_zero) - static_cast<int>(trailing_zero) << std::endl;
        bstream.write_bit(ZERO);   // Second control bit
        bstream.write_bits(delta_vale >> trailing_zero, 64 - static_cast<int>(leading_zero) - static_cast<int>(trailing_zero));
    } else {
        leading_zero = lz;
        trailing_zero = tz;
        ...
        int sigbits = 64 - static_cast<int>(leading_zero) - static_cast<int>(trailing_zero);
...
void XORAppender::append(int64_t timestamp, double value){
    ...
    if(num_samples == 0){
        ...
        current_delta_timestamp = timestamp - this->timestamp;
        int64_t delta_delta_timestamp = current_delta_timestamp - this->delta_timestamp;
...
bool XORAppender::bit_range(int64_t delta_delta_timestamp, int num){
    return ((-((1<<(num-1))-1) <= delta_delta_timestamp) && (delta_delta_timestamp <= 1<<(num-1)));
}
