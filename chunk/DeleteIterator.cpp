...
DeleteIterator::DeleteIterator(std::unique_ptr<ChunkIteratorInterface> && it,
...
std::pair<int64_t, double> DeleteIterator::at() const{
    return it->at();
}
    ...
    Outer:
        while(it->next()){
            ...
            while(itvls_begin != itvls_end){
                if(itvls_begin->in_bounds(p.first))
                    goto Outer;
...
bool DeleteIterator::error() const{
    return it->error();
}
